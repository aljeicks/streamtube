			...
			<tr>
				...
				foreach($multi_link as $link){
					if(strpos($link, 'http') !== false){ //is a url
						...
						?>
                        <a class="multilink-btn btn btn-sm btn-default bordercolor2hover bgcolor2hover <?php if(isset($_GET['link']) && $_GET['link']==$link_count){ echo 'current-link'; } ?>" href="<?php echo add_query_arg( 'link', $link_count, get_permalink(get_the_ID()) ); ?>"><i class="fa fa-play"></i> <?php echo $temp_title?$temp_title:__('Link ').($link_number+1); ?> </a>
                        <?php
						$temp_title = '';
						$link_count++;
						$link_number++;
